...
class DummyData {
    ...
    static func initDataForRestaurants() -> [Restaurant] {
        /*
            OLD DATA
        ...
        let size = restaurantNames.count
        var counter = 0
        for i in 0..<size {
            let name = restaurantNames[i]
            let image = restaurantImages[i]
            let location = restaurantLocations[i]
            let type = restaurantTypes[i]
         
            let restaurant = Restaurant(name, location, type, image)
        ...
        */
        ...
        let data = DataHelper.getFromDatabase()
        if data.count > 0 {
            return data
        }
